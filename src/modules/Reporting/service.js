...
const totalOrderAndVariationsSoldService = async (startDate, endDate) => {
  try {
    ...
    const variationsSoldResult = await OrderModel.aggregate(
      totalVariationsSoldPipeline
    );
    const totalVariationsSold =
      variationsSoldResult.length > 0
        ? variationsSoldResult[0].totalVariationsSold
        : 0;
    ...
    const netSales = await OrderModel.aggregate([
      ...
      },
      {
        $group: {
          _id: null,
          totalNetSales: {
            $sum: { $subtract: ["$totalPrice", "$discountAmount"] },
          },
        },
      },
    ]);
///hufhish
    ...
    const avgGrossDailySales = grossSales.length
      ? grossSales[0].totalGrossSales / totalDays
      : 0;

    // Calculate average net daily sales
    const avgNetDailySales = netSales.length
      ? netSales[0].totalNetSales / totalDays
      : 0;
    ...
    return {
      ...
      avgNetDailySales,
      ordersPlacedToday,
      itemsPurchasedToday: itemsPurchasedToday.length
        ? itemsPurchasedToday[0].totalItemsPurchased
        : 0,
      refunded: refundedOrders.length ? refundedOrders[0].totalRefunded : 0,
      deliveryCharge: totalDeliveryCharge.length
        ? totalDeliveryCharge[0].totalDeliveryCharge
        : 0,
    };
  } catch (error) {
