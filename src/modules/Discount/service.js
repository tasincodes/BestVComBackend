...
const generateCouponService = async (couponInfo) => {
  try {
    ...
    if (
      discountType === "fixed" &&
      (couponAmount == null || couponAmount === undefined)
    ) {
...
const updateCouponServicebyId = async (id, updatedInfo) => {
  try {
    ...
    const couponUpdates = await couponModel.findByIdAndUpdate(id, updatedInfo, {
      new: true,
    });
...
const getCouponByCodeService = async (couponCode) => {
  try {
    ...
    return {
      success: false,
      error: "Failed to retreive coupons by coupon name",
    };
  }
};

const getDiscountByCoupon = async (
  couponId,
  totalPrice,
  requestedProducts,
  userId
  ...
  const requestedProductData = await productModel.find({
    _id: { $in: requestedProducts },
  });
  requestedProductData.forEach((product) => {
    if (!coupon.usageRestriction.products.includes(product._id.toString())) {
      throw new BadRequest(
        `Product ${product.productName} is not eligible for this coupon`
      );
    }
    if (
      coupon.usageRestriction.excludeProducts.includes(product._id.toString())
    ) {
      throw new BadRequest(
        `Product ${product.productName} is excluded from this coupon`
      );
    }
  });

  const categoryIds = [
    ...new Set(requestedProductData.map((product) => product.categoryId)),
  ];
  ...
  categoryData.forEach((category) => {
    if (!coupon.usageRestriction.categories.includes(category._id.toString())) {
      throw new BadRequest(
        `Category ${category.categoryName} is not eligible for this coupon`
      );
    }
    if (
      coupon.usageRestriction.excludeCategories.includes(
        category._id.toString()
      )
    ) {
      throw new BadRequest(
        `Category ${category.categoryName} is excluded from this coupon`
      );
    }
  });
  ...
  try {
    ...
    if (discountType === "fixed" || discountType === "percentage") {
      coupons = await couponModel.find({
        "general.discountType": discountType,
      });
    } else {
      coupons = [];
    }
